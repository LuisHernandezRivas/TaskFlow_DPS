        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej. Crear base de datos"
              value={titulo}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                className="w-full px-4 py-2 border rounded-lg mt-1 bg-white"
                value={prioridad}
          ...
          <div>
            ...
            <div className="max-h-36 overflow-y-auto border rounded-lg mt-1 p-2 bg-white space-y-2">
              {usuarios.map((usuario) => (
                <label key={usuario.email} className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={usuariosSeleccionados.includes(usuario.email)}
                    onChange={() => handleToggleUsuario(usuario.email)}
                  />
                  <span>
                    {usuario.name} <span className="text-gray-500">({usuario.role})</span>
                  </span>
                </label>
              ))}
            </div>
            {usuarios.length === 0 && (
              <p className="text-sm text-red-600 mt-2">No hay usuarios con rol usuario registrados.</p>
            )}
          </div>
